...
(
    ...
    parent_category int REFERENCES product_category (category_id) ON DELETE CASCADE,
    level           int         NOT NULL
);
...
(
    ...
    customer        int        NOT NULL REFERENCES customer (customer_id) ON DELETE NO ACTION,
    address         int        NOT NULL REFERENCES shipping_address (address_id) ON DELETE NO ACTION,
    total_price     numeric(2) NOT NULL,
    total_amount    int        NOT NULL,
    date_created    date       NOT NULL,
    purchase_status int        NOT NULL REFERENCES purchase_status (status_id) ON DELETE NO ACTION
);

CREATE TABLE IF NOT EXISTS purchase_products
(
    purchase_products int PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    purchase_id       int        NOT NULL REFERENCES purchase (purchase_id) ON DELETE NO ACTION,
    product_id        int        NOT NULL REFERENCES product (product_id) ON DELETE NO ACTION,
